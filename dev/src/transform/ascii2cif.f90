program ascii2cif
  ...
  if ( len_trim(file_in) == 0 ) then
    stop "first argument must contain .ascii filename"
  end if
  call get_command_argument(2,file_out)
  if ( len_trim(file_out) == 0 ) then
    stop "second argument must contain .cif filename"
  end if
  ...
  call write_cif(trim(file_out), nat, rxyz, alat, atomnames, comment)
end program ascii2cif
